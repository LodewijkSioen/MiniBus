...
public class TracingTests
{
    ...
    public void SetUp()
    {
        ...
        {
            ShouldListenTo = source => source.Name == "Caravelle",
    ...
    public async Task SuccessfulDispatch_ActivityHasCorrectNameAndTags()
    {
        ...
        Assert.That(activity.OperationName, Is.EqualTo("caravelle.dispatch SimpleHandler"));
        Assert.That(activity.GetTagItem("caravelle.request.type"), Is.EqualTo(typeof(SimpleHandler.Request).FullName));
        Assert.That(activity.GetTagItem("caravelle.result.type"), Is.EqualTo(typeof(SimpleHandler.Response).FullName));
    ...
    public async Task NotFoundResult_SpanStatusIsNotError()
    {
        ...
        Assert.That(_activities, Has.Count.EqualTo(1));
        var activity = _activities[0];
        Assert.That(activity.GetTagItem("caravelle.result.type"), Is.EqualTo(typeof(global::Caravelle.NotFoundResult).FullName));
        Assert.That(activity.Status, Is.EqualTo(ActivityStatusCode.Unset));
    }
    ...
    public async Task InvalidResult_SpanStatusIsNotError()
    {
        ...
        Assert.That(_activities, Has.Count.EqualTo(1));
        var activity = _activities[0];
        Assert.That(activity.GetTagItem("caravelle.result.type"), Is.EqualTo(typeof(global::Caravelle.ValidationResult).FullName));
        Assert.That(activity.Status, Is.EqualTo(ActivityStatusCode.Unset));
    }
    ...
    public void OperationCanceledException_DoesNotSetErrorStatus_AndDoesNotRecordExceptionEvent()
    {
        ...
        var activity = _activities[0];
        Assert.That(activity.Status, Is.EqualTo(ActivityStatusCode.Unset));
        Assert.That(activity.GetTagItem("caravelle.result.status"), Is.EqualTo("Canceled"));
