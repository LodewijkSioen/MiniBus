...
public class FinallyPipelineHandler
{
    ...
    {
        var validation = new global::Caravelle.ValidationResult();
        if (entity.Mode == "invalid")
            validation.Add(new global::Caravelle.ValidationError("invalid mode", "INVALID_MODE"));
        return validation;
    }
    ...
    public void Finally(
        Request request,
        Entity? entity,
        global::Caravelle.ValidationResult? validation,
        Response? response,
...
public class FinallyMethodIntegrationTests
{
    ...
    public async Task Finally_Runs_WhenDispatchReturnsInvalid()
    {
        ...
        Assert.That(result.Value, Is.TypeOf<global::Caravelle.ValidationResult>());
        Assert.That(probe.Observations, Has.Count.EqualTo(1));
